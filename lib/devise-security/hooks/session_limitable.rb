...
Warden::Manager.after_set_user except: :fetch do |record, warden, options|
  if record.should_limit_session && record.respond_to?(:update_unique_session_id!) && warden.authenticated?(options[:scope])
...
Warden::Manager.after_set_user only: :fetch do |record, warden, options|
  ...
  if record.should_limit_session && record.respond_to?(:unique_session_id) && warden.authenticated?(scope) && options[:store] != false
